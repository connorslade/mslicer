...
fn main() -> Result<()> {
    ...
    if flipped_winding > 0 {
        println!("{path:?} {flipped_winding} flipped faces");
    }

    // println!(
    //     "{{ unwelded_vertices: {unwelded_vertices}, holes: {holes}, degenerative_faces: {degenerative_faces}, flipped_winding: {flipped_winding} }}"
    // );
