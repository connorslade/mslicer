use egui::{
    Context, DragAndDrop, Id, Label, LayerId, Order, Sense, Ui, UiBuilder, Widget,
...
pub fn collection(app: &mut App, ctx: &Context, ui: &mut Ui, collection: Option<u32>) {
    for j in 0..app.project.models.len() {
        ...
        if ctx.is_being_dragged(eid) {
            ...
            let response = model_entry(app, ui, j, false);
...
pub fn collection_entry(app: &mut App, ui: &mut Ui, idx: usize) {
    let group = &mut app.project.collections[idx];
